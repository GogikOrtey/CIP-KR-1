...
# Заменяем пропущенные значения на -999999 и 999999
data['Левая граница'] = data['Левая граница'].replace(np.nan, '-999999')
data['Правая граница'] = data['Правая граница'].replace(np.nan, '999999')
...
# # Шаг 4: Вывести общее количество людей которым больше всего нравится 1 фильм, 2 фильм и т.д. (стоит 1 место)
# fav_films = data[['Star Wars: Часть 1', 'Star Wars: Часть 2', 'Star Wars: Часть 3', 'Star Wars: Часть 4', 'Star Wars: Часть 5', 'Star Wars: Часть 6']].apply(lambda x: (x == 1).sum())

# # Шаг 5: Сгруппировать людей по полу и вывести график зависимости пола от оценки каждой части
# gender_pref = data.groupby('Gender')[['Star Wars: Часть 1', 'Star Wars: Часть 2', 'Star Wars: Часть 3', 'Star Wars: Часть 4', 'Star Wars: Часть 5', 'Star Wars: Часть 6']].mean()
# gender_pref.transpose().plot(kind='bar')

# # Шаг 6: Взять среднее значение оценки каждого фильма по диапазону возраста и вывести на графике зависимость возраста от средней оценки каждого фильма
# age_pref = data.groupby('Age')[['Star Wars: Часть 1', 'Star Wars: Часть 2', 'Star Wars: Часть 3', 'Star Wars: Часть 4', 'Star Wars: Часть 5', 'Star Wars: Часть 6']].mean()
